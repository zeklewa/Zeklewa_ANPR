...
float check_score(Vec2f can_vl,Vec2f can_vr,Vec2f can_hu,Vec2f can_hd,Mat img, float ratio)
{
...
{
//printf("v2, number of channel is %d\n",img_src.channels());
Mat img;
		...
		for (int j=0;j<num_vr;j++)
			...
			for (int k=0;k<num_hu;k++)
				...
				for (int h=0;h<num_hd;h++)
					...
					can_hu=lines_hu[k];
					can_hd=lines_hd[h];
					float score_tmp=check_score(can_vl,can_vr,can_hu,can_hd,img_src,ratio);
					//printf("score=%f\n",score_tmp);
